...
class DDPG():
    ...
    def get_behaviours(self, task, iter_in_episode):
        """
        Get behaviours of a trained agent for one episode
        """
        ...
        t = 0
        state = self.task.reset()
        while (t < iter_in_episode) and (not done):
            action = self.actor_target.model.predict(np.reshape(state, (1, self.state_size)))[0] # shape (a_dim,)
